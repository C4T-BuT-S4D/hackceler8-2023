...
BOSS_TITLE = 'rAIbbit Mk.I "Temporal Anomaly"'
HEALTHBAR_LEN = 1240
...
class DanmakuSystem:
    def __init__(self, player, boss, is_server):
        ...
        if not self.is_server:
            ...
            self.gui["clock_face"] = arcade.Sprite(
                "resources/villain/clock_face.png", scale=1.5
            )
            self.gui["clock_face"].alpha = 0
            self.gui["clock_face"].set_position(640, 640)
            self.gui["clock_hand1"] = arcade.Sprite(
                "resources/villain/clock_hand.png", scale=1.5
            )
            self.gui["clock_hand1"].alpha = 0
            self.gui["clock_hand1"].set_position(640, 640)
            self.gui["clock_hand2"] = arcade.Sprite(
                "resources/villain/clock_hand.png", scale=1.5
            )
            self.gui["clock_hand2"].alpha = 0
            self.gui["clock_hand2"].set_position(640, 640)
        ...
        self.player.anomaly_speed_multiplier = 1
    ...
    def anomaly(self):
        if not self.is_server:
            self.anim_coroutines.add(self.anomaly_anim())

    def anomaly_anim(self):
        if self.boss_phase == 1:
            self.gui["clock_face"].alpha = 64
            self.gui["clock_hand1"].alpha = 64
            for i in range(60 * 2):
                progress = i / (60 * 2 - 1)
                self.gui["clock_hand1"].angle = 360 * progress
                yield
            self.gui["clock_face"].alpha = 0
            self.gui["clock_hand1"].alpha = 0
        else:
            self.gui["clock_face"].alpha = 64
            self.gui["clock_hand1"].alpha = 64
            self.gui["clock_hand2"].alpha = 64
            for i in range(60 * 2):
                progress = i / (60 * 2 - 1)
                self.gui["clock_hand1"].angle = 360 * progress
                self.gui["clock_hand2"].angle = -360 * progress
                yield
            self.gui["clock_face"].alpha = 0
            self.gui["clock_hand1"].alpha = 0
            self.gui["clock_hand2"].alpha = 0
        ...
        if self.game_running:
            if not self.boss.destructing:
                if (
                    ...
                    and self.ticks % 3 == 0
                    and self.player.anomaly_speed_multiplier > 0
                ):
                    self.player_shoot(
