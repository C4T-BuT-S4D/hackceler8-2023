...
class BossScriptPhase1(BossScript):
    ...
    def reflect(self):
        while True:
            for i in range(5):
                self.system.shoot(
                    ReflectBullet(
                        self.system, self.x, self.y, 300, 30 * (i - 2) + 90, scale=3
                    )
                )
    ...
    def above(self):
        while True:
            for i in range(41):
                self.system.shoot(
                    AnomalyBullet(self.system, self.x, self.y, 300, 3 * (i - 20) + 90)
                )
            yield from sleep_ticks(30)
    ...
    def above(self):
        while True:
            for i in range(41):
                self.system.shoot(
                    AnomalyBullet(
                        self.system, self.x, self.y, 300, 3 * (i - 20) + 90, scale=2
                    )
                )
            yield from sleep_ticks(30)
...
class Bullet(arcade.Sprite):
    ...
    def move_simple(self):
        ...
        x += (
            self.speed
            * self.system.bullet_speed_multiplier
            * constants.TICK_S
...
class AnomalyBullet(SimpleBullet):
    def __init__(
        self, system, x, y, speed, angle, shape="oval", scale=1, apply_anomaly=False
    ):
        super().__init__(
            system, x, y, speed, angle, hue=0, saturation=0.0, shape=shape, scale=scale
        )
...
class PlayerBullet(SimpleBullet):
    ...
    def move_simple(self):
        ...
        x += (
            self.speed
            * self.system.player.anomaly_speed_multiplier
            * constants.TICK_S
            * math.cos(math.radians(self.angle))
        )
        y += (
