...
class Inventory:
    ...
    def update_display(self):
        ...
        self.stats_box.add(
            self._stats_text_area(
                width=400, height=30, text="HEALTH:% 3.02f" % self.game.player.health
            )
        )
        quit_btn = QuitButton(
        ...
        self.stats_box.add(
            self._stats_text_area(
                width=440, height=30, text="    PLAY TIME: %s" % self.display_time
            )
        )
        ...
        self.manager.add(
            arcade.gui.UIAnchorWidget(
                anchor_x="center_x", anchor_y="center_y", child=self.main_box
            )
        )
