...
class BossScriptPhase1(BossScript):
    ...
    def maze_guide(self):
        self.system.shoot(
            RectMazeGuideBullet(self.system, 640, 1280, 640, 30 + 96, 1280, 16)
        )
        self.system.shoot(
            RectMazeGuideBullet(self.system, 0, 640, 640 - 48, 640, 16, 1280)
        )
        self.system.shoot(
            RectMazeGuideBullet(self.system, 1280, 640, 640 + 48, 640, 16, 1280)
        )
        yield from sleep_ticks(60 * 2)
        self.system.shoot(
            RectMazeOuterWallBullet(
                self.system, 640 - 96 * 5.5, 30 + 96 * 3, 16, 96 * 6
            )
        )
        self.system.shoot(
            RectMazeOuterWallBullet(
        ...
        self.system.shoot(
            RectMazeOuterWallBullet(self.system, 640, 30 + 96 * 6, 96 * 11, 16)
        )
    ...
    def maze(self):
        ...
        while True:
            for x, y in RECT_MAZES[mazei][0]:
                self.system.shoot(
                    RectMazeBullet(
                        self.system, 640 + 96 * (x - 5), 30 + 96 * y, 96, 16, angle
                    )
                )
    ...
    def rings(self):
        while True:
            ...
            for i in range(20):
                self.system.shoot(
                    SimpleBullet(self.system, self.x, self.y, 300, 18 * i + angle)
                )
                    ...
                    if CIRC_MAZES[mazei][0][j - 1][i] == "1":
                        for k in range(5):
                            ...
                            self.system.shoot(
                                CircMazeBullet(
                                    self.system,
                                    640 + r * math.cos(math.radians(angle)),
                                    380 + r * math.sin(math.radians(angle)),
                                    angle + 180,
                                    delete_after_speedup,
                                )
                            )
                    if j != 4 and CIRC_MAZES[mazei][1][j - 1][i] == "1":
                        for k in range(6):
                            ...
                            self.system.shoot(
                                CircMazeBullet(
                                    self.system,
                                    640 + radius * math.cos(math.radians(angle)),
                                    380 + radius * math.sin(math.radians(angle)),
                                    angle + 180,
                                    -1,
                                )
                            )
    ...
    def rings(self):
        while True:
            ...
            for i in range(20):
                self.system.shoot(
                    SimpleBullet(
                        self.system, self.x, self.y, 200, 18 * i + angle, hue=0
                    )
                )
...
class Bullet(arcade.Sprite):
    ...
    def __init__(
        ...
        if key in texture_cache:
            ...
            image = image.crop(
                (BASE_SHAPE_X[shape], 0, BASE_SHAPE_X[shape] + BULLET_SIZE, BULLET_SIZE)
            )
            for i, px in enumerate(image.getdata()):
                alpha = px[3]
                lightness = colorsys.rgb_to_hls(px[0] / 255, px[1] / 255, px[2] / 255)[
                    1
                ]
                xx = i % BULLET_SIZE
                yy = i // BULLET_SIZE
                red, green, blue = [
                    int(n * 255)
                    for n in colorsys.hls_to_rgb(hue / 360, lightness, saturation)
                ]
...
class SimpleBullet(Bullet):
    def __init__(
        self, system, x, y, speed, angle, hue=60, saturation=1.0, shape="oval", scale=1
    ):
...
class CircMazeGuideBullet(SimpleBullet):
    def __init__(self, system, x, y, rmin, rmax, angle):
        super().__init__(
            system,
            x - rmax * math.cos(math.radians(angle)),
            y - rmax * math.sin(math.radians(angle)),
            0,
            angle,
            hue=120,
            shape="arrow",
...
class PlayerBullet(SimpleBullet):
    def __init__(self, system, x, y, speed, angle):
        super().__init__(
            system, x, y, speed, angle, hue=60, saturation=1.0, shape="arrow", scale=1
        )
        self.intangible = False
        self.alpha = 255
        self.scale = 1
