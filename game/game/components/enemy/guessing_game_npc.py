...
class GuessingGameNpc(Npc):
    ...
    def __init__(self, coords, name, walk_data):
        super().__init__(
            coords, name, walk_data, scale=1, tileset_path="resources/NPCs/Duck_NPC.tmx"
        )
        self.item = magic_items.Item(
            coords=None, name="item_purple", display_name="Purple key", color="purple"
        )
        outline = [
    ...
    def dialogue(self):
        ...
        if GuessingGameNpc.solved:
            self.display_textbox(
                "I don't have any more quacks to give you.\nGo quack somebody else."
            )
            return
    ...
    def _game(self):
        ...
        def resp_process(resp: str):
            ...
            if resp != num:
                self.display_textbox(
                    "I LIED! I was actually thinking of %s, quack." % num
                )
                return
            else:
                self.display_textbox(
