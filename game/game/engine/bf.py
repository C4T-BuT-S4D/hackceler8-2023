...
class Interpreter:
    ...
    def execute_until_return(self):
        ...
        while (
            not self.stopped
            and a is None
            and self.executed_instructions < self.max_instructions
        ):
            try:
                a = self.step()
            except Exception as e:
    ...
    def execute_until_end(self):
        ...
        if self.executed_instructions >= self.max_instructions:
            logging.critical("Exceeded max number of steps")
        while not self.stopped and self.executed_instructions < self.max_instructions:
            try:
                self.step()
            except Exception as e:
                logging.critical(f"Execution error: {e}")
        ...
        match op:
            ...
            case 91:  # [
                ...
                if self.pc in self.jmps:
                    self.pc = (
                        self.jmps[self.pc]
                        if (self.dp in self.data and self.data[self.dp] == 0)
                        else self.pc + 1
                    )
                return
            case 93:  # ]
