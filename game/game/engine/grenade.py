...
class GrenadeSystem:
    ...
    def __init__(self, game, targets=None):
        ...
        logging.debug(f"initialized grenade system with {len(self.targets)} targets.")
    ...
    def _check_empty(self, player, obj):
        ...
        space = hitbox.Hitbox(
            [
                hitbox.Point(x0 + 1, y1 - 1),
                hitbox.Point(x1 - 1, y1 - 1),
                hitbox.Point(x1 - 1, y0 + 1),
                hitbox.Point(x0 + 1, y0 + 1),
            ]
        )
