...
class BankerNPC(Npc):
    ...
    def update_text(self):
        self.text = (
            f"Do you want to buy some stuff? You currently have "
    ...
    def process_buying(self, resp: str):
        ...
        if bought_success:
            ...
            self.display_textbox(
                f"Hmmm living a bit above our means there jimbo, "
    ...
    def process_selling(self, resp: str):
        ...
        if sold_success:
            ...
            self.display_textbox(
                f"You either don't have that item or it's not "
    ...
    def process_inventory(self, resp: str):
        ...
        self.display_textbox(have_now, process_fun=self.basic_interaction)
        ...
        match resp:
            ...
            case "Selling":
                if self.shop.player_inventory_total != 0:
                    self.selling("")
                self.display_textbox(
                    "You don't have any items!", process_fun=self.basic_interaction
                )
            case "Inventory":
    ...
    def selling(self, resp):
        self.display_textbox(
            "Sure thing, I'll be glad to buy some stuff back from ya",
            choices=[
                f"{i}: {self.shop.items[i].value}"
                for i in self.shop.player_inventory
                if self.shop.player_inventory[i] > 0
            ],
