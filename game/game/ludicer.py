...
class Ludicer:
    def __init__(
        self, net, is_server, debug=True, eager_level_load=True, load_file="save_state"
    ):
        self.mutex = Lock()
        self.net = net
    ...
    def dump_items(self):
        save_string = b"\xfe\xfe".join(
            [i.dump() for i in self.items] + [str(time.time()).encode()]
        )
    ...
    def query_llm(self, text):
        ...
        def _query_llm(text):
            ...
            if not serverless_client:
                ...
                msg = json.dumps(
                    {"llm_resp": resp, "codeword_guessed": codeword_guessed}
                ).encode()
    ...
    def tick(self):
        ...
        if (
            arcade.key.SLASH in self.newly_pressed_keys
            and not self.save_cooldown
        ...
        if self.save_cooldown_timer != 0:
            self.save_cooldown_timer = max(
                self.save_cooldown_timer - constants.TICK_S, 0
            )
        else:
            self.save_cooldown = False
        ...
        for o in self.objects:
            ...
            if o.nametype == "NPC" or o.nametype == "Boss":
                ...
                if o.get_rect().expand(20).collides(self.player.get_rect()):
                    ...
                    if (
                        self.danmaku_system is None
                        and arcade.key.E in self.newly_pressed_keys
