        ...
        while True:
            ...
            for face in range(6):
                for i in range(4):
                    ...
                    bullet = CubeFaceBullet(
                        self.system,
                        x,
                        y,
                        FACE_SIZE / 2,
                        FACE_SIZE / 2,
                        CUBES2[cubei][face * 4 + i],
                        delay=delay,
                    )
                    self.cube_faces.append(bullet)
                    self.system.shoot(bullet)
            cube_laser = CubeFaceBullet(
                self.system, 640, BASE_Y, 1280, FACE_SIZE * 3, 0, delay=delay
            )
        ...
        while True:
            ...
            for row in range(5):
                for col in range(5):
                    ...
                    self.system.shoot(
                        SquareBullet(
                            self.system, cx, cy, 100, theta, radius, hue, saturation
                        )
                    )
            colori = (colori + 1) % 6
    ...
    def bigfan(self):
        ...
        while True:
            ...
            for i in range(27):
                ...
                self.system.shoot(
                    SimpleBullet(
                        self.system, self.x, self.y, 400, angle, hue, saturation
                    )
                )
            colori = (colori + 1) % 6
    ...
    def rotate(self, rotation):
        ...
        for perm in PERM2[rotation]:
            ...
            for i in range(len(perm) - 1):
                self.cube_faces[perm[i]].set_position(
                    *self.cube_faces[perm[i + 1]].position
                )
...
class SquareBullet(SimpleBullet):
    ...
    def run(self):
        ...
        while True:
            ...
            self.angle += 0.2
            self.theta += 0.2
            x = self.cx + self.cur_radius_mult * self.radius * math.cos(
                math.radians(self.theta)
            )
            y = self.cy + self.cur_radius_mult * self.radius * math.sin(
                math.radians(self.theta)
            )
            self.set_position(x, y)
            yield
