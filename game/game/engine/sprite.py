        ...
        for flashing in (False, True):
            ...
            if (flipped and not can_flip) or (flashing and not can_flash):
                ...
                textures[flipped][flashing] = load_spritesheet(
                    tileset, flipped, flashing, repeat_size
                )
    return textures
    ...
    for y in range(tileset.tile_count // tileset.columns):
        for x in range(tileset.columns):
            ...
            if repeat_size is not None:
                ...
                texture = arcade.Texture(
                    name=f"{tileset.image},{x},{y},{flashing},{flipped}", image=repeated
                )
...
class Sprite:
    def __init__(
        self, tileset_path: str, can_flip=False, can_flash=False, repeat_size=None
    ):
        ...
        self.textures = load_textures(
            self.tileset, self.can_flip, self.can_flash, self.repeat_size
        )
    ...
    def set_texture(self, img_path):
        ...
        self.textures = load_textures(
            self.tileset, self.can_flip, self.can_flash, repeat_size=None
        )
