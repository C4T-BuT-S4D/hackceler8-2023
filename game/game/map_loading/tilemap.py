...
class BasicTileMap:
    ...
    def parse_spike_layer(self, layer):
        ...
        for row in range(len(layer.data)):
            for column in range(len(layer.data[row])):
                if layer.data[row][column] == 0:
                    continue
                coords = pytiled_parser.OrderedPair(
                    column * 16 + 32, max_y - row * 16 + 16
                )
    ...
    def parse_object(self, o, max_y):
        ...
        if o.name == "Fire":
            ...
            self.dynamic_artifacts.append(
                cls(coords, o.size, min_distance=md, damage=dmg)
            )
            ...
            self.objs.append(
                enemy_types.ENEMY_TYPES[o.name](
                    coords,
                    o.name,
                    props.get("damage", None),
                    props.get("respawn", False),
                    props.get("respawn_tick", None),
                    props.get("walk_data", ""),
                    props.get("control_inverter", False),
                )
            )
            ...
            p = moving_platform.MovingPlatform(
                coords,
                o.name,
                props.get("x_speed", 0.0),
                props.get("y_speed", 0.0),
                props.get("min_dx", -20),
                props.get("max_dx", 20),
                props.get("min_dy", -20),
                props.get("max_dy", 20),
            ...
            self.static_objs.append(
                portal.Portal(
                    coords, o.size, o.name, dest, x_speed=x_speed, y_speed=y_speed
                )
            )
